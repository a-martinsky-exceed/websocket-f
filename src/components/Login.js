...
export class Login extends Component {
  render() {
    ...
    const disabled = !(this.props.username.length > 0 && this.props.password.length > 0)
    ...
    return (
      <div className="window login">
        ...
        {![200, null].includes(this.props.status) &&
          <div className="error">
            {existingError ? 'Wrong username or password' :
              this.props.status > 0 ? `Error ${this.props.status}` : ''}
          </div>
        }

        <div className="submit form-group row">
          <div className="col-sm-10">
            <button type='submit' disabled={disabled} className='btn btn-primary' onClick={this.props.onSubmit}>Login</button>
          </div>
        </div>
      </div>
    )
